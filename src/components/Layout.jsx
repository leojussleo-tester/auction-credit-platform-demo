...
const LANG_KEY = 'auction-ui-language'
...
export default function Layout({ children, route, onLogout }) {
  ...
  function switchLang(nextLang) {
    ...
    window.dispatchEvent(new CustomEvent('auction-language-change', { detail: nextLang }))
  }

  useEffect(() => { document.documentElement.lang = lang === 'vi' ? 'vi' : 'en' }, [lang])
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 lg:px-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <a href="#/" className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-auction-gold">{t.product}</p>
              </div>
            </a>
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex rounded-2xl border border-white/15 bg-white/[0.06] p-1">
                <button onClick={() => switchLang('vi')} className={`rounded-xl px-3 py-1.5 text-xs font-black ${lang === 'vi' ? 'bg-auction-gold text-black' : 'text-slate-300'}`}>VN</button>
                <button onClick={() => switchLang('en')} className={`rounded-xl px-3 py-1.5 text-xs font-black ${lang === 'en' ? 'bg-auction-gold text-black' : 'text-slate-300'}`}>EN</button>
              </div>
              <Badge>{roleLabel(role)}</Badge>
              <Badge>{tierLabel(currentUser, currentMemberLevel)}</Badge>
              <button onClick={onLogout} className="rounded-2xl border border-white/15 bg-white/[0.06] px-4 py-2 text-sm font-black text-white hover:border-auction-gold/50">{t.logout}</button>
            </div>
          </div>
          ...
          <nav className="flex gap-2 overflow-x-auto pb-1">
            {navItems.map((item) => {
              ...
              return <a key={item.href} href={item.href} className={`whitespace-nowrap rounded-2xl px-4 py-2 text-sm font-bold transition duration-300 ${active ? 'bg-auction-gold text-black shadow-neon' : 'text-slate-300 hover:-translate-y-0.5 hover:bg-white/10 hover:text-white'}`}>{item.label}</a>
            })}
          </nav>
        </div>
      </header>
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[300px_1fr] lg:px-6">
        <aside className="space-y-4 lg:sticky lg:top-28 lg:h-fit">
          <div className="section-card">
            <p className="label">{roleLabel(role)} {t.profile}</p>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
              <div className="soft-card p-3"><p className="text-slate-300">{t.available}</p><p className="font-black text-white">{money(currentUser?.wallet?.available)}</p></div>
              <div className="soft-card p-3"><p className="text-slate-300">{t.pending}</p><p className="font-black text-auction-gold">{money(currentUser?.wallet?.pending)}</p></div>
            </div>
            {role === 'admin' ? <div className="mt-4 rounded-2xl border border-auction-gold/25 bg-auction-gold/10 p-4 text-sm leading-6 text-slate-200">{t.adminNote}</div> : null}
            <button onClick={resetDemo} className="btn-secondary mt-4 w-full">{t.reset}</button>
          </div>

          <div className="soft-card p-5">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-auction-gold">{t.core}</p>
            <p className="mt-3 text-sm leading-6 text-slate-300">{t.coreText}</p>
          </div>
        </aside>
