        ...
        <div className="mt-4 grid gap-4 md:grid-cols-[1fr_220px]">
          <div className="soft-card p-3">
            ...
            <label className="mt-4 block">
              ...
              <input className={`field !py-2 ${error ? '!border-rose-400 !bg-rose-500/10' : ''}`} type="file" accept="image/*" disabled={readonly || success} onChange={(e) => onUploadBill(e.target.files?.[0])} />
            </label>
...
export default function WalletFixed() {
  ...
  function submitWithdraw(event) {
    ...
    if (!withdrawAmount || withdrawAmount <= 0) return notify('Số Credit muốn rút phải lớn hơn 0.', 'error')
    if (withdrawAmount > Number(currentUser.wallet.available || 0)) return notify('Không đủ Available Credit để rút. Pending Credit không được tính vào số dư có thể rút.', 'error')
