  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="muted mt-3 max-w-3xl">Quản lý user, KYC, seller permission, product verification, room setup, winner paid/failed, penalty và seller settlement.</p>
      </div>
      ...
      <PolicyBox title="Admin Demo Flow">
        Test nhanh: vào Bid Room để đặt bid → vào Admin Dashboard để Set Ended → Mark as Paid hoặc Failed Payment → Release Seller Settlement nếu Paid. Fee 10% được trừ trước khi chuyển cho seller.
      </PolicyBox>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_0.9fr]">
        ...
        <form onSubmit={createRoom} className="glass-card p-6">
          ...
          <div className="space-y-4">
            <label>
              <span className="label">Verified Product</span>
              <select className="field" value={roomForm.productId} onChange={(e) => setRoomForm({ ...roomForm, productId: e.target.value })}>
                <option value="">Manual / No Product</option>
                {verifiedProducts.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}
              </select>
            </label>
            <label>
              <span className="label">Seller</span>
              <select className="field" value={roomForm.sellerId} onChange={(e) => setRoomForm({ ...roomForm, sellerId: e.target.value })}>
                {sellerUsers.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}
              </select>
            </label>
            <label>
              <span className="label">Room Title</span>
              <input className="field" value={roomForm.title} onChange={(e) => setRoomForm({ ...roomForm, title: e.target.value })} placeholder="Optional if product selected" />
            </label>
            <div className="grid gap-4 md:grid-cols-2">
              <label>
                <span className="label">Room Level</span>
                <select className="field" value={roomForm.roomLevel} onChange={(e) => setRoomForm({ ...roomForm, roomLevel: e.target.value })}>
                  <option>Basic</option><option>Pro</option><option>VIP</option>
                </select>
              </label>
              <label>
                <span className="label">Status</span>
                <select className="field" value={roomForm.status} onChange={(e) => setRoomForm({ ...roomForm, status: e.target.value })}>
                  <option>Upcoming</option><option>Live</option><option>Ended</option>
                </select>
              </label>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <label>
                ...
                <input className="field" type="number" value={roomForm.startingPrice} onChange={(e) => setRoomForm({ ...roomForm, startingPrice: e.target.value })} />
              </label>
              <label>
                <span className="label">Min Increment</span>
                <input className="field" type="number" value={roomForm.minIncrement} onChange={(e) => setRoomForm({ ...roomForm, minIncrement: e.target.value })} />
              </label>
            </div>
      ...
      <section className="glass-card p-6">
        <div className="mb-5">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-auction-gold">Room Operations</p>
          <h2 className="mt-2 text-2xl font-black text-white">Set level/status & xử lý winner</h2>
        </div>
        <div className="space-y-4">
            ...
            return (
              <div key={room.id} className="soft-card p-4">
                <div className="grid gap-4 xl:grid-cols-[1fr_1.2fr]">
                  <div>
                    <div className="flex flex-wrap gap-2">
                      <Badge>{room.roomLevel}</Badge><Badge>{room.status}</Badge><Badge tone="default">{room.paymentStatus}</Badge><Badge tone="default">{room.settlementStatus}</Badge>
                    </div>
                  ...
                  <div className="flex flex-wrap items-start gap-2">
                    <select className="field !w-32 !px-3 !py-2" value={room.roomLevel} onChange={(e) => adminUpdateRoom(room.id, { roomLevel: e.target.value })}>
                      <option>Basic</option><option>Pro</option><option>VIP</option>
                    </select>
                    <select className="field !w-36 !px-3 !py-2" value={room.status} onChange={(e) => adminUpdateRoom(room.id, { status: e.target.value })}>
                      <option>Upcoming</option><option>Live</option><option>Ended</option>
                    </select>
                ...
                {paidBid ? (
                  <div className="mt-4 rounded-2xl border border-auction-gold/20 bg-auction-gold/10 p-4 text-sm text-slate-200">
                    Paid amount {money(paidBid.amount)} · Platform fee 10% = {money(Math.ceil(paidBid.amount * room.commissionRate))} · Seller settlement = {money(paidBid.amount - Math.ceil(paidBid.amount * room.commissionRate))}
                  </div>
                ) : null}
              </div>
            )
      ...
      <section className="glass-card p-6">
        ...
        <div className="mt-5 space-y-3">
          {state.activityLog.map((log) => (
            <div key={log.id} className="soft-card p-4 text-sm text-slate-300">
              <span className="font-mono text-xs text-slate-500">{formatDateTime(log.time)}</span> · {log.message}
            </div>
          ))}
        </div>
      </section>
